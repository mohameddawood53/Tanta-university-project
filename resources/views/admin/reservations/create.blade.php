...
@section('content')
    <div class="container">
        <h1 class="text-center p-5">إضافة ميعاد جديد</h1>
        <div class="form-group">
            <form action="/reservations" method="post">

                @csrf
                <label for="date">الميعاد</label>
                <input type="date" dir="rtl" name="date" id="date" class="form-control @error('date') is-invalid @enderror">
                @error("date")
                <div class="alert alert-danger">{{$message}}</div>
                @enderror
                <label for="max_number">أقصي عدد من الطلبة للتسجيل</label>
                <input type="number"  name="max_number" id="max_number" class="form-control @error('max_number') is-invalid @enderror" placeholder="أقصي عدد من الطلبة">
                @error("max_number")
                <div class="alert alert-danger">{{$message}}</div>
                @enderror
                <div class="text-center">
                    <input class="btn btn-success m-3" type="submit" value="إدخال">
                </div>
